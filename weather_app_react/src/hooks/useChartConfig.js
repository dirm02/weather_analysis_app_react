...
function makeSeries(i, dataType, datums, useR) {
  ...
  return {
    label:`Series ${i + 1}`,
    data: [...new Array(length)].map((_, idx) => {
      let x;
      if (dataType === "ordinal") {
        x = `Ordinal Group ${start + idx}`;
      } else if (dataType === "time") {
        x = new Date(startDate.getTime() + 60 * 1000 * 60 * 24 * idx);
      ...
      const r = !useR ? undefined : rMax - Math.floor(Math.log(Math.random() * (Math.pow(distribution, rMax) - rMin) + rMin) / Math.log(distribution));
      return {
        primary: x,
        secondary: y,
        radius: r,
